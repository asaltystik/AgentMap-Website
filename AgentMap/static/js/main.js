...
function updateTimeZones() {
    ...
    Object.entries(zones).forEach(([zone, offset]) => {
        let element = document.querySelector(`#${zone}`);
        if (element) element.textContent = getTimeInTimeZone(offset);
    })
}
        ...
        if (color) {
            ...
            element.addEventListener('mouseover', function() {
                ...
                if (element.classList.contains('stateName')) {
                    ...
                    let path = document.querySelector('path[id="' + element.getAttribute('id') + '"]');let circle = document.querySelector('circle[id="' + element.getAttribute('id') + '"]');
